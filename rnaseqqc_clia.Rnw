...
sCounts <- read.csv2(fileHT, sep='\t', header=F)
sCounts <- sCounts[1:(dim(sCounts)[1]-5),] # remove last 5 lines, hold no genes
nGenes <- dim(sCounts)[1]
nCounts <- sum(sCounts$V2)
eGenes <- length(which(sCounts$V2>0))

## CallableLoci
callableLoci <- read.csv2(fileCLoci, sep='')
callableTotal <- sum(callableLoci$nBases)
callableN <- callableLoci$nBases[2]
@
...
library(yaml)
sample <- yaml.load_file(sampleMeta)[[sampleID]]
@
...
runinfoGB <- ifelse(sample$run_info$total_yield_gb>=90, "$\\\\text{\\\\color{red}{\\\\rlap{$\\\\checkmark$}}}\\\\square$", "$\\\\square$")
runinfoQ30 <- ifelse(sample$run_info$percent_q_score_30>=75, "$\\\\text{\\\\color{red}{\\\\rlap{$\\\\checkmark$}}}\\\\square$", "$\\\\square$")
runinfoPhiX <- ifelse(sample$run_info$percent_phix_alg>=0.05, "$\\\\text{\\\\color{red}{\\\\rlap{$\\\\checkmark$}}}\\\\square$", "$\\\\square$")
runinfoPhiXErr <- ifelse(sample$run_info$phix_error_rate<2.5, "$\\\\text{\\\\color{red}{\\\\rlap{$\\\\checkmark$}}}\\\\square$", "$\\\\square$")
@
